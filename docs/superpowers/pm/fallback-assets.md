...
This audit logs in as `admin / Admin@123`, walks the scripted business pages, captures nine screenshots, and writes `visual-audit.json`. The JSON currently records zero local page or console errors, zero page-level horizontal overflow on every audited route, and no forbidden stale wording.
...
| `06-interactive-teaching-stage.png` | Whiteboard-priority stage, remote whiteboard, desktop share, teacher focus | Clause ♦27 stage and layout proof |
...
| `06-interactive-teaching.png` | Full interactive teaching state after toggles | Backup overview image for Clause ♦27 |
...
- Do not use the interactive teaching images to imply real RTC, real remote desktop, teacher audio detection, or real remote whiteboard protocol.
