...
test('passes the July 9 smart education demo route gates', async ({ page }) => {
  ...
  await expect(page.getByTestId('teaching-stage')).toContainText('模拟白板共享中')
  await page.getByTestId('teaching-desktop-share').click()
  await expect(page.getByTestId('teaching-stage')).toContainText('桌面画面状态共享中')
  ...
  await page.getByTestId('blackboard-complete-button').click()
  await expect(page.getByTestId('blackboard-completion-notice')).toContainText('已完成 1 个课堂活动')
  await expect(page.getByTestId('blackboard-completed-list')).toContainText('选词填空')
  await expect(page.getByTestId('blackboard-completed-list')).toContainText('A. 指南针')
  await expect(page.getByTestId('blackboard-completed-list')).toContainText('文本输入')
  await expect(page.getByTestId('blackboard-completed-list')).toContainText('已完成')
  await page.getByTestId('blackboard-stem-input').fill('老师临时修改后的题干。')
  await expect(page.getByTestId('blackboard-preview')).toContainText('老师临时修改后的题干。')
  await page.getByTestId('blackboard-open-activity-activity-1').click()
  await expect(page.getByTestId('blackboard-preview')).toContainText('选词填空：____是中国古代四大发明之一')
  await expect(page.getByTestId('blackboard-preview')).not.toContainText('老师临时修改后的题干。')

  await page.getByTestId('blackboard-type-judgement').click()
  await page.getByLabel('课堂文本').fill('判断：鲸鱼是鱼类，这句话不正确。')
  await page.getByTestId('blackboard-parse-button').click()
  await expect(page.getByTestId('blackboard-preview')).toContainText('错误')
  await expect(page.getByTestId('blackboard-preview')).toContainText('正确答案')

  await page.getByTestId('blackboard-type-choice').click()
  await page.getByLabel('课堂文本').fill('下列哪一项属于可再生能源？A. 煤炭 B. 风能 C. 石油 正确答案：风能')
  await page.getByTestId('blackboard-parse-button').click()
  await expect(page.getByTestId('blackboard-preview')).toContainText('风能')
  await expect(page.getByTestId('blackboard-preview')).toContainText('正确答案')

  await page.getByTestId('blackboard-type-cloze').click()
  await page.getByLabel('课堂文本').fill('嗯 这个 长江是中国第一长河。答案：长江')
  await page.getByTestId('blackboard-parse-button').click()
  await expect(page.getByTestId('blackboard-preview')).not.toContainText('嗯')
  await expect(page.getByTestId('blackboard-preview')).not.toContainText('这个')
